...
contract MockMorphoBlue is IMorpho {
  ...
  function seedSupply(MarketParams memory params, uint256 assets) external {
    ...
    uint256 shares = m.totalSupplyShares == 0 ? assets : (assets * uint256(m.totalSupplyShares)) / uint256(m.totalSupplyAssets);
  ...
  function supply(
    ...
    if (assets > 0) {
      IERC20(params.loanToken).safeTransferFrom(msg.sender, address(this), assets);
      sharesMinted = m.totalSupplyShares == 0 ? assets : (assets * uint256(m.totalSupplyShares)) / uint256(m.totalSupplyAssets);
      assetsSupplied = assets;
    } else {
      // shares path
      sharesMinted = shares;
      assetsSupplied = (shares * uint256(m.totalSupplyAssets)) / (uint256(m.totalSupplyShares) == 0 ? 1 : uint256(m.totalSupplyShares));
  ...
  function withdraw(
    ...
    if (assets > 0) {
      sharesBurned = (assets * uint256(m.totalSupplyShares)) / (uint256(m.totalSupplyAssets) == 0 ? 1 : uint256(m.totalSupplyAssets));
      assetsWithdrawn = assets;
    } else {
      sharesBurned = shares;
      assetsWithdrawn = (shares * uint256(m.totalSupplyAssets)) / (uint256(m.totalSupplyShares) == 0 ? 1 : uint256(m.totalSupplyShares));
    }
  ...
  function borrow(
    ...
    require(m.lastUpdate != 0, "no market");
    if (assets > 0) {
      sharesBorrowed = m.totalBorrowShares == 0 ? assets : (assets * uint256(m.totalBorrowShares)) / uint256(m.totalBorrowAssets);
      assetsBorrowed = assets;
    } else {
      sharesBorrowed = shares;
      assetsBorrowed = (shares * uint256(m.totalBorrowAssets)) / (uint256(m.totalBorrowShares) == 0 ? 1 : uint256(m.totalBorrowShares));
    }
  ...
  function repay(
    ...
    Position storage p = positions[id][onBehalf];
    if (assets > 0) {
      sharesRepaid = m.totalBorrowShares == 0 ? assets : (assets * uint256(m.totalBorrowShares)) / uint256(m.totalBorrowAssets);
      assetsRepaid = assets;
    } else {
      sharesRepaid = shares;
      assetsRepaid = (shares * uint256(m.totalBorrowAssets)) / (uint256(m.totalBorrowShares) == 0 ? 1 : uint256(m.totalBorrowShares));
    }
