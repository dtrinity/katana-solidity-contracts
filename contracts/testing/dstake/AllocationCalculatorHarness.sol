...
contract AllocationCalculatorHarness {
  ...
  ) external pure returns (uint256[] memory deficits, uint256[] memory surpluses, uint256 totalDeficit, uint256 totalSurplus) {
    return AllocationCalculator.calculateDeficitsAndSurpluses(currentAllocations, targetAllocations);
  }

  function splitAmountEvenly(uint256 totalAmount, uint256 vaultCount) external pure returns (uint256[] memory amounts) {
  ...
  function distributeRemainder(
    ...
    return AllocationCalculator.distributeRemainder(amounts, totalAmount, weights, remainder);
  }
  ...
  function calculateVaultAllocation(
    ...
    return AllocationCalculator.calculateVaultAllocation(vaultBalance, totalBalance, scaleFactor);
  }

  function validateTargetAllocations(uint256[] memory targetAllocations) external pure returns (bool isValid, uint256 totalBps) {
  ...
  function calculateOptimalWithdrawal(
    ...
    return AllocationCalculator.calculateOptimalWithdrawal(targetAmount, vaultBalances, targetAllocations);
  }
}
