...
library AllocationCalculator {
  ...
  ) internal pure returns (uint256[] memory deficits, uint256[] memory surpluses, uint256 totalDeficit, uint256 totalSurplus) {
   ...
   */
  function splitAmountEvenly(uint256 totalAmount, uint256 vaultCount) internal pure returns (uint256[] memory amounts) {
    if (vaultCount == 0) {
   ...
   */
  function validateTargetAllocations(uint256[] memory targetAllocations) internal pure returns (bool isValid, uint256 totalBps) {
    totalBps = 0;
