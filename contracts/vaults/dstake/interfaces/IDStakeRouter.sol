...
interface IDStakeRouter {
  /**
   * @notice Handles the conversion of deposited dStable asset into a chosen `strategy share`
  ...
  /**
   * @notice Handles the conversion of a `strategy share` back into the dStable asset for withdrawal.
   * @dev Called by `DStakeToken._withdraw()`.
   * @dev The router coordinates pulling the required `strategy share` from the collateral vault
  ...
  /**
   * @notice Exchanges collateral from one strategy share to another.
   * @dev Only callable by authorized strategy rebalancers.
   * @param fromStrategyVault The strategy vault to exchange from.
   * @param toStrategyVault The strategy vault to exchange to.
   * @param amount The amount of dStable equivalent to exchange.
   * @param minToStrategyShareAmount The minimum amount of toStrategyVault assets expected.
   */
  function rebalanceStrategiesByValue(address fromStrategyVault, address toStrategyVault, uint256 amount, uint256 minToStrategyShareAmount) external;
   ...
   * @param strategyVaults Array of strategy vault addresses to deposit into
   * @param shares Array of share amounts to deposit into each strategy vault
   */
  function solverDepositShares(address[] calldata strategyVaults, uint256[] calldata shares) external;
   ...
   * @param strategyVaults Array of strategy vault addresses to withdraw from
   * @param shares Array of share amounts to withdraw from each strategy vault
   * @param receiver The address that will receive the withdrawn dStable asset
   * @param owner The original owner initiating the withdrawal
   */
