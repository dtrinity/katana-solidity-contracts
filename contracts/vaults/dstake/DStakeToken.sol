...
contract DStakeToken is Initializable, ERC4626Upgradeable, AccessControlUpgradeable, SupportsWithdrawalFee {
   ...
   *      (converting dSTABLE to strategy shares) to the router.
   */
   ...
   *      (converting strategy shares back to dSTABLE) to the router.
  ...
  function solverDepositShares(
    ...
    // Calculate total assets by converting strategy shares to asset amounts
    uint256 totalAssetAmount = 0;
    for (uint256 i = 0; i < strategyVaults.length; i++) {
      if (shares[i] > 0) {
        // Use the strategy vault's previewMint to convert shares to assets
        uint256 assetAmount = IERC4626(strategyVaults[i]).previewMint(shares[i]);
    ...
    // Delegate to router's solver deposit method
    router.solverDepositShares(strategyVaults, shares);
   ...
   * @param assets Array of asset amounts to withdraw from each strategy vault (net amounts requested)
   ...
   *      The router returns gross amounts, and this function handles fee calculation.
   * @param strategyVaults Array of strategy vault addresses to withdraw from
   * @param strategyShares Array of share amounts to withdraw from each strategy vault
   * @param maxShares Maximum dSTAKE shares to burn (slippage protection)
   * @param receiver Address to receive the withdrawn assets (after fees)
   * @param owner Address that owns the dSTAKE shares being burned
  ...
  function solverWithdrawShares(
    ...
    // Calculate expected gross assets by converting strategy shares to assets
