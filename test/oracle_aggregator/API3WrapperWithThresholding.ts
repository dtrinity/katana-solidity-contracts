  ...
  describe(`API3WrapperWithThresholding for ${currency}`, () => {
    ...
    describe("Asset pricing with thresholding", () => {
      it("should return original price when no threshold is set", async function () {
        const thresholdAssetKeys = Object.keys(fixtureResult.assets.api3ThresholdAssets);
        ...
        // Get the existing MockAPI3OracleAlwaysAlive instance from the fixture data
        const mockProxy = await ethers.getContractAt("MockAPI3OracleAlwaysAlive", assetData.proxy);

        // Set a test price
        const testPrice = ethers.parseUnits("1", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        await mockProxy.setMock(testPrice);

        // Get price info
        const { price: actualPrice, isAlive } = await api3WrapperWithThresholding.getPriceInfo(testAsset);

        // Verify price and status - should return the original price since no threshold config is set for this asset
