...
export async function getOracleAggregatorFixture(currency: string) {
  return deployments.createFixture(async ({ deployments }): Promise<OracleAggregatorFixtureResult> => {
    ...
    try {
      ...
      wrapperWithThresholdingDeployment = await hre.deployments.get(wrapperIds.wrapperWithThresholding);
      compositeWrapperWithThresholdingDeployment = await hre.deployments.get(wrapperIds.compositeWrapperWithThresholding);
      api3WrapperDeployment = await hre.deployments.get(wrapperIds.api3Wrapper);
      api3WrapperWithThresholdingDeployment = await hre.deployments.get(wrapperIds.api3WrapperWithThresholding);
      api3CompositeWrapperWithThresholdingDeployment = await hre.deployments.get(wrapperIds.api3CompositeWrapperWithThresholding);
    } catch (error) {
      throw new Error(
    ...
    const api3Wrapper = (await hre.ethers.getContractAt("API3Wrapper", api3WrapperDeployment.address)) as API3Wrapper;
    ...
    for (const [assetAddress, feedAddress] of Object.entries(oracleConfig.redstoneOracleAssets.plainRedstoneOracleWrappers)) {
    ...
    for (const [assetAddress, assetConfig] of Object.entries(oracleConfig.redstoneOracleAssets.redstoneOracleWrappersWithThresholding)) {
    ...
    for (const [assetAddress, proxyAddress] of Object.entries(oracleConfig.api3OracleAssets.plainApi3OracleWrappers)) {
    ...
    for (const [assetAddress, assetConfig] of Object.entries(oracleConfig.api3OracleAssets.api3OracleWrappersWithThresholding)) {
    ...
    for (const [assetAddress, assetConfig] of Object.entries(oracleConfig.api3OracleAssets.compositeApi3OracleWrappersWithThresholding)) {
