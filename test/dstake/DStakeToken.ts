...
import { DStakeCollateralVaultV2, DStakeRouterV2, DStakeTokenV2, ERC20 } from "../../typechain-types";
...
DSTAKE_CONFIGS.forEach((config: DStakeFixtureConfig) => {
  describe.skip(`DStakeTokenV2 for ${config.DStakeTokenV2Symbol}`, () => {
    ...
    beforeEach(async () => {
      ...
      adapter = MetaMorphoConversionAdapter__factory.connect(adapterAddress, deployer);
    ...
    describe("Initialization & State", () => {
      it("Should set immutable dStable address via asset()", async () => {
        expect(await DStakeTokenV2.asset()).to.equal(await dStableToken.getAddress());
      });
      ...
      it("Should have collateralVault and router set from fixture", async () => {
        expect(await DStakeTokenV2.collateralVault()).to.equal(collateralVaultAddress);
      ...
      it("New instance withdrawalFeeBps should be zero by default", async () => {
        ...
        const fresh = await ethers.getContractAt("DStakeTokenV2", deployResult.address);
    ...
    describe("Role-Based Access Control & Configuration", () => {
      ...
      describe("migrateCore", () => {
        const deployNewCore = async () => {
          const vaultFactory = await ethers.getContractFactory("DStakeCollateralVaultV2", user1);
          const deployedVault = await vaultFactory.deploy(DStakeTokenV2Address, await dStableToken.getAddress());
          await deployedVault.waitForDeployment();

          const routerFactory = await ethers.getContractFactory("DStakeRouterV2", user1);
          const deployedRouter = await routerFactory.deploy(DStakeTokenV2Address, await deployedVault.getAddress());
        ...
        it("Should allow admin to migrate router and collateral vault", async () => {
          ...
          await expect(DStakeTokenV2.connect(user1).migrateCore(newRouterAddress, newVaultAddress))
        ...
        it("Should revert if non-admin calls migrateCore", async () => {
          await expect(DStakeTokenV2.connect(deployer).migrateCore(routerAddress, collateralVaultAddress)).to.be.revertedWithCustomError(
            DStakeTokenV2,
        ...
        it("Should revert if any address is zero", async () => {
          await expect(DStakeTokenV2.connect(user1).migrateCore(ZeroAddress, collateralVaultAddress)).to.be.revertedWithCustomError(
            DStakeTokenV2,
            "ZeroAddress",
          );

          await expect(DStakeTokenV2.connect(user1).migrateCore(routerAddress, ZeroAddress)).to.be.revertedWithCustomError(
            DStakeTokenV2,
            "ZeroAddress",
          );
        });
        ...
        it("Should revert when router collateral vault does not match", async () => {
          ...
          await expect(
            DStakeTokenV2.connect(user1).migrateCore(await deployedRouter.getAddress(), collateralVaultAddress),
          ).to.be.revertedWithCustomError(DStakeTokenV2, "RouterCollateralMismatch");
        });

        it("Should revert when router is configured for a different token", async () => {
          const routerFactory = await ethers.getContractFactory("DStakeRouterV2", user1);
          const mismatchedRouter = await routerFactory.deploy(user1.address, collateralVaultAddress);
          await mismatchedRouter.waitForDeployment();

          await expect(
            DStakeTokenV2.connect(user1).migrateCore(await mismatchedRouter.getAddress(), collateralVaultAddress),
          ).to.be.revertedWithCustomError(DStakeTokenV2, "RouterTokenMismatch");
        });
      });

      describe("Role Management", () => {
        it("Should allow admin to grant and revoke FEE_MANAGER_ROLE", async () => {
          await expect(DStakeTokenV2.connect(user1).grantRole(FEE_MANAGER_ROLE, deployer.address)).to.not.be.reverted;
          expect(await DStakeTokenV2.hasRole(FEE_MANAGER_ROLE, deployer.address)).to.be.true;
          await expect(DStakeTokenV2.connect(user1).revokeRole(FEE_MANAGER_ROLE, deployer.address)).to.not.be.reverted;
          expect(await DStakeTokenV2.hasRole(FEE_MANAGER_ROLE, deployer.address)).to.be.false;
        });
      });

      describe("setWithdrawalFee", () => {
        it("Should allow fee manager to set withdrawal fee", async () => {
          await expect(DStakeTokenV2.connect(user1).setWithdrawalFee(100)).to.emit(router, "WithdrawalFeeSet").withArgs(0, 100);
          expect(await DStakeTokenV2.withdrawalFeeBps()).to.equal(100);
        });

        it("Should revert if non-fee-manager sets withdrawal fee", async () => {
          await expect(DStakeTokenV2.connect(deployer).setWithdrawalFee(100)).to.be.revertedWithCustomError(
            DStakeTokenV2,
            "AccessControlUnauthorizedAccount",
          );
        });
        ...
        it("Should revert if fee exceeds maxWithdrawalFeeBps", async () => {
          await expect(DStakeTokenV2.connect(user1).setWithdrawalFee(10001)).to.be.revertedWithCustomError(router, "InvalidWithdrawalFee");
        });
    ...
    describe("ERC4626 Core Functionality (Deposits & Minting)", () => {
      ...
      it("totalAssets delegates correctly to collateralVault", async () => {
        ...
        await dStableToken.connect(user1).approve(DStakeTokenV2Address, assetsToDeposit);
        await DStakeTokenV2.connect(user1).deposit(assetsToDeposit, user1.address);
      ...
      it("previewDeposit returns expected shares", async () => {
        expect(await DStakeTokenV2.previewDeposit(assetsToDeposit)).to.equal(assetsToDeposit);
      });

      it("maxDeposit returns uint256 max", async () => {
        expect(await DStakeTokenV2.maxDeposit(user1.address)).to.equal(ethers.MaxUint256);
      });
      ...
      describe("deposit function", () => {
        ...
        it("should revert with ERC20InvalidReceiver when receiver is zero", async () => {
          await stable.mint(user1.address, assetsToDeposit);
          await dStableToken.connect(user1).approve(DStakeTokenV2Address, assetsToDeposit);
          await expect(DStakeTokenV2.connect(user1).deposit(assetsToDeposit, ZeroAddress))
        ...
        it("should revert on insufficient balance", async () => {
          await dStableToken.connect(user1).approve(DStakeTokenV2Address, assetsToDeposit);
          await expect(DStakeTokenV2.connect(user1).deposit(assetsToDeposit, user1.address)).to.be.reverted;
        });
        ...
        it("should mint shares and emit Deposit event", async () => {
          ...
          await expect(DStakeTokenV2.connect(user1).deposit(assetsToDeposit, user1.address))
      ...
      describe("mint function", () => {
        it("should mint shares and emit Deposit event via mint", async () => {
          ...
          await dStableToken.connect(user1).approve(DStakeTokenV2Address, assetsToProvide);
          await expect(DStakeTokenV2.connect(user1).mint(sharesToMint, user1.address))
            .to.emit(DStakeTokenV2, "Deposit")
            .withArgs(user1.address, user1.address, assetsToProvide, sharesToMint);
          expect(await DStakeTokenV2.balanceOf(user1.address)).to.equal(sharesToMint);
        });
      });
    });
    ...
    describe("ERC4626 Core Functionality (Withdrawals & Redeeming)", () => {
      ...
      it("previewWithdraw returns expected shares", async () => {
        expect(await DStakeTokenV2.previewWithdraw(assetsToDeposit)).to.equal(shares);
      });

      it("previewRedeem returns expected assets", async () => {
        expect(await DStakeTokenV2.previewRedeem(shares)).to.equal(assetsToDeposit);
      });

      it("maxWithdraw returns deposit amount", async () => {
        expect(await DStakeTokenV2.maxWithdraw(user1.address)).to.equal(assetsToDeposit);
      });
      ...
      it("allows zero-amount withdraw probes", async () => {
        await expect(DStakeTokenV2.connect(user1).withdraw(0, user1.address, user1.address))
      ...
      it("allows zero-share redeem probes", async () => {
        await expect(DStakeTokenV2.connect(user1).redeem(0, user1.address, user1.address))
          .to.emit(DStakeTokenV2, "Withdraw")
          .withArgs(user1.address, user1.address, user1.address, 0, 0);
      });
      ...
      it("should withdraw assets and burn shares", async () => {
        ...
        const sharesToBurn = await DStakeTokenV2.previewWithdraw(assetsToWithdraw);
        await expect(DStakeTokenV2.connect(user1).withdraw(assetsToWithdraw, user1.address, user1.address))
          .to.emit(DStakeTokenV2, "Withdraw")
      ...
      it("should redeem shares and transfer assets", async () => {
        ...
        await expect(DStakeTokenV2.connect(user1).redeem(sharesToRedeem, user1.address, user1.address))
          .to.emit(DStakeTokenV2, "Withdraw")
    ...
    describe("ERC4626 Withdrawals & Redeeming with Fees", () => {
      ...
      beforeEach(async () => {
        ...
        await dStableToken.connect(user1).approve(DStakeTokenV2Address, grossDeposit);
      ...
      it("should withdraw assets with fee deducted", async () => {
        ...
        await DStakeTokenV2.connect(user1).withdraw(assetsToDeposit, user1.address, user1.address);
      ...
      it("should redeem shares with fee deducted", async () => {
        ...
        await DStakeTokenV2.connect(user1).redeem(shares, user1.address, user1.address);
      ...
      it("previewWithdraw returns expected shares including fee", async () => {
        ...
        expect(await DStakeTokenV2.previewWithdraw(assetsToDeposit)).to.equal(expectedGrossAmount);
      });
      ...
      it("previewRedeem returns expected assets after fee", async () => {
        ...
        expect(await DStakeTokenV2.previewRedeem(shares)).to.equal(expectedAssets);
      });
      ...
      it("preserves ERC4626 conversion invariants with a withdrawal fee", async () => {
        ...
        const roundTrip = await DStakeTokenV2.convertToShares(await DStakeTokenV2.convertToAssets(shares));
      ...
      it("maxWithdraw returns net amount after fee and allows full withdrawal", async () => {
        ...
        await expect(DStakeTokenV2.connect(user1).withdraw(netMax, user1.address, user1.address))
          .to.emit(DStakeTokenV2, "Withdraw")
    ...
    describe("Security: Unauthorized withdrawal protection", () => {
      ...
      beforeEach(async () => {
        ...
        await dStableToken.connect(user1).approve(DStakeTokenV2.target, assetsToDeposit);
      ...
      it("should prevent unauthorized withdrawal without allowance", async () => {
        ...
        ).to.be.revertedWithCustomError(DStakeTokenV2, "ERC20InsufficientAllowance");
      });
