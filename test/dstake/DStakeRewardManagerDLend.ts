...
DSTAKE_CONFIGS.forEach((config: DStakeFixtureConfig) => {
  describe.skip(`DStakeRewardManagerDLend for ${config.DStakeTokenSymbol}`, function () {
    ...
    const rewardTokenSymbol = config.dStableSymbol === "dUSD" ? "sfrxUSD" : "stETH";
    const dStableTokenId = config.dStableSymbol === "dUSD" ? DUSD_TOKEN_ID : DETH_TOKEN_ID;
