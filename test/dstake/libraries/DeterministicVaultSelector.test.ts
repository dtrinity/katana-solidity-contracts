...
describe("DeterministicVaultSelector Library Tests", function () {
  ...
  describe("calculateUnderallocations", function () {
    ...
    it("Should revert with mismatched array lengths", async function () {
      ...
      await expect(
        harness.calculateUnderallocations(currentBps, targetBps)
  ...
  describe("calculateOverallocations", function () {
    ...
    it("Should revert with mismatched array lengths", async function () {
      ...
      await expect(
        harness.calculateOverallocations(currentBps, targetBps)
      ).to.be.revertedWithCustomError(harness, "ArrayLengthMismatch");
    });
  });
  ...
  describe("selectTopUnderallocated", function () {
    it("Should select vault with highest underallocation for deposits", async function () {
      ...
      const [selectedVaults, selectedIndices] = await harness.selectTopUnderallocated(
        vaults, currentBps, targetBps, count
      );
    ...
    it("Should revert with mismatched array lengths", async function () {
      ...
      await expect(
        harness.selectTopUnderallocated(vaults, currentBps, targetBps, 1)
      ).to.be.revertedWithCustomError(harness, "ArrayLengthMismatch");
    });
    ...
    it("Should revert when requesting more vaults than available", async function () {
      ...
      await expect(
        harness.selectTopUnderallocated(vaults, currentBps, targetBps, 3)
    ...
    it("Should revert with zero count", async function () {
      ...
      await expect(
        harness.selectTopUnderallocated(vaults, currentBps, targetBps, 0)
      ).to.be.revertedWithCustomError(harness, "InvalidSelectionCount");
    });
  ...
  describe("selectTopOverallocated", function () {
    it("Should select vault with highest overallocation for withdrawals", async function () {
      ...
      const [selectedVaults, selectedIndices] = await harness.selectTopOverallocated(
        vaults, currentBps, targetBps, count
      );
  ...
  describe("Utility Functions", function () {
    describe("hasNonZeroDeltas", function () {
      it("Should return true when deltas exist", async function () {
        const deltas = [100000, 0, 50000];
        const result = await harness.hasNonZeroDeltas(deltas);
        expect(result).to.be.true;
      });

      it("Should return false when no deltas exist", async function () {
        const deltas = [0, 0, 0];
        const result = await harness.hasNonZeroDeltas(deltas);
    ...
    describe("calculateTotalDelta", function () {
      it("Should sum all deltas correctly", async function () {
        const deltas = [100000, 200000, 50000];
        const result = await harness.calculateTotalDelta(deltas);
        expect(result).to.equal(350000);
      });
      ...
      it("Should handle array with zeros", async function () {
        const deltas = [100000, 0, 0, 200000];
        const result = await harness.calculateTotalDelta(deltas);
        expect(result).to.equal(300000);
      });
    });

    describe("getVaultsWithNonZeroDeltas", function () {
      it("Should filter vaults with non-zero deltas", async function () {
        const vaults = [vault1, vault2, vault3, vault4];
        const deltas = [100000, 0, 50000, 0];
        
        const [vaultsWithDeltas, indices] = await harness.getVaultsWithNonZeroDeltas(vaults, deltas);
        
        expect(vaultsWithDeltas).to.have.lengthOf(2);
        expect(vaultsWithDeltas[0]).to.equal(vault1);
        expect(vaultsWithDeltas[1]).to.equal(vault3);
        expect(indices[0]).to.equal(0);
        expect(indices[1]).to.equal(2);
      });

      it("Should return empty arrays when no non-zero deltas", async function () {
        const vaults = [vault1, vault2, vault3];
        const deltas = [0, 0, 0];
        
        const [vaultsWithDeltas, indices] = await harness.getVaultsWithNonZeroDeltas(vaults, deltas);
        
        expect(vaultsWithDeltas).to.have.lengthOf(0);
        expect(indices).to.have.lengthOf(0);
      });

      it("Should revert with mismatched array lengths", async function () {
        const vaults = [vault1, vault2];
        const deltas = [100000, 200000, 300000]; // Different length
        
        await expect(
          harness.getVaultsWithNonZeroDeltas(vaults, deltas)
        ).to.be.revertedWithCustomError(harness, "ArrayLengthMismatch");
      });
    });
  });
  ...
  describe("Gas Efficiency Tests", function () {
    it("Should have reasonable gas consumption for selectTopUnderallocated", async function () {
      ...
      const tx = await harness.selectTopUnderallocated.populateTransaction(
        vaults, currentBps, targetBps, count
      );
    ...
    it("Should scale linearly with vault count", async function () {
      ...
      const tx3 = await harness.selectTopUnderallocated.populateTransaction(
        vaults3, currentBps3, targetBps3, 1
      );
      ...
      const tx4 = await harness.selectTopUnderallocated.populateTransaction(
        vaults4, currentBps4, targetBps4, 1
      );
