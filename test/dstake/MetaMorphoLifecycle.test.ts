...
describe("dSTAKE MetaMorpho Lifecycle", function () {
  ...
  const setupLifecycleFixture = deployments.createFixture(async (hre) => {
    ...
    const [ownerSigner, aliceSigner, bobSigner, charlieSigner, treasurySigner, managerSigner] = await ethers.getSigners();

    // Get deployed contracts
    const { contract: dStableBaseContract } = await getTokenContractForSymbol(hre, deployer, config.dStableSymbol);

    // Cast to proper contract type to access mint function
    const dStableContract = await ethers.getContractAt("ERC20StablecoinUpgradeable", dStableBaseContract.target);
    ...
    const metaMorphoVaultContract = await ethers.getContractAt("MockMetaMorphoVault", metaMorphoVaultDeployment.address);

    const adapterDeployment = await deployments.get(`MetaMorphoConversionAdapter_${config.dStableSymbol}`);
    const adapterContract = await ethers.getContractAt("MetaMorphoConversionAdapter", adapterDeployment.address);
    ...
    const rewardManagerDeployment = await deployments.get(`DStakeRewardManagerMetaMorpho_${config.DStakeTokenV2Symbol}`);
    const rewardManagerContract = await ethers.getContractAt("DStakeRewardManagerMetaMorpho", rewardManagerDeployment.address);
  ...
  describe("Complete dSTAKE Lifecycle with MetaMorpho", function () {
    ...
    it("Phase 5: Reward compounding", async function () {
      ...
      await expect(rewardManager.connect(alice).compoundRewards(compoundAmount, [rewardToken.target], alice.address)).to.emit(
        rewardManager,
        "RewardCompounded",
      );
    ...
    it("Phase 6: Partial withdrawal", async function () {
      ...
      await expect(dStakeToken.connect(bob).redeem(withdrawShares, bob.address, bob.address)).to.emit(dStakeToken, "Withdraw");
    ...
    it("Phase 7: Simulate vault fee changes", async function () {
      ...
      await expect(dStakeToken.connect(alice).redeem(sharesToRedeem, alice.address, alice.address)).to.be.reverted;
