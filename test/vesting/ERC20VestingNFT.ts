...
const createVestingFixture = deployments.createFixture(async ({ deployments }) => {
  ...
  const dstakeToken = (await ethers.getContractAt("TestMintableERC20", mockDStakeTokenV2.address)) as TestMintableERC20;
  ...
  const vestingNFTDeployment = await deployments.deploy("TestERC20VestingNFT", {
    ...
    args: ["Test dSTAKE Vesting NFT", "TEST-dVEST", mockDStakeTokenV2.address, VESTING_PERIOD, MAX_TOTAL_SUPPLY, 0, deployer],
    log: false,
  });
