...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const minterComplete = await ensureMinterRole(hre, tokenAddresses.dETH, issuerAddress, executor);

  let allOperationsComplete = minterComplete;

  console.log(`  🔐 Migrating IssuerV2 roles to governance...`);
  const rolesComplete = await migrateIssuerRolesIdempotent(hre, issuerAddress, deployerSigner, governanceMultisig, executor);
  if (!rolesComplete) allOperationsComplete = false;

  if (!allOperationsComplete) {
    const flushed = await executor.flush(`Setup IssuerV2 (dETH): governance operations`);
