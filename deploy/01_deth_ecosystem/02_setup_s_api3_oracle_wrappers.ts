...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  const baseCurrencyUnit = BigInt(10) ** BigInt(config.oracleAggregators.ETH.priceDecimals);
  const baseCurrency = config.oracleAggregators.ETH.baseCurrency;

  // Deploy API3Wrapper for plain oracle feeds
  const api3WrapperDeployment = await hre.deployments.deploy(ETH_API3_ORACLE_WRAPPER_ID, {
    from: deployer,
  ...
  const api3WrapperWithThresholdingDeployment = await hre.deployments.deploy(ETH_API3_WRAPPER_WITH_THRESHOLDING_ID, {
    from: deployer,
    args: [baseCurrency, baseCurrencyUnit],
  ...
  const api3CompositeWrapperDeployment = await hre.deployments.deploy(ETH_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID, {
    from: deployer,
    args: [baseCurrency, baseCurrencyUnit],
